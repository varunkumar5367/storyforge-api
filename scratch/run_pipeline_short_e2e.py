...
async def main():
    ...
    # Read the story file
    story_path = ROOT_DIR / "story_short.txt"
    if not story_path.exists():
        print(f"Error: story_short.txt not found at {story_path}")
        return
        
    with open(story_path, "r", encoding="utf-8") as f:
        story_text = f.read()
        
    job_id = "e2e-short-test-" + str(int(time.time()))
    ...
    await database.create_job(
        ...
        story_filename="story_short.txt",
    ...
    print("\n" + "="*50)
    print("E2E SHORT PIPELINE RUN COMPLETED")
    ...
    if video_path.exists():
        ...
        # Let's inspect the video dimensions using ffprobe if available
        import subprocess
        try:
            cmd = ["ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries", "stream=width,height", "-of", "csv=p=0", str(video_path)]
            out = subprocess.check_output(cmd, text=True).strip()
            print(f"Video dimensions (ffprobe): {out}")
        except Exception as e:
            print(f"Could not run ffprobe to verify video dimensions: {e}")
    else:
